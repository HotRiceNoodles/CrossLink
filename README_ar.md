...
- 🎁 **نواة مفتوحة المصدر سخيّة** — إصدار Community (Apache 2.0) يضم 47 إجراءً يشمل MCP
  و RBAC وإحصاءات التوجيه وقواعد الأخطاء، وإدارة قواعد الحواجز (كلمات مفتاحية / طول /
  كشف بيانات الاعتماد)، و Playground التفاعلي، وقوالب الأوامر، ورموز PAT محددة النطاق.
  Pro يضيف إدارة الأسرار وتنبيهات الحواجز وإعادة تشغيل التصحيح و DataLens ودرع الوكيل؛
  Enterprise يضيف متعدد المؤسسات والتدقيق وإدارة الميزانيات.
...
- **تخزين مؤقت للاستجابات** — تخزين عبر Redis مع TTL لكل نموذج، وضغط gzip، وعزل مفتاح
  التخزين لكل مستخدم
- **توليد الفيديو** — `POST /v1/videos` مع استعلام الحالة وتنزيل المحتوى، عبر نفس
  سلسلة التراجع المستخدمة للمحادثة
- **قوالب الأوامر (Prompt)** — كتالوج قوالب يُدار عبر `/admin/api/templates`؛ يكتشفه
  حاملو المفاتيح عبر `GET /v1/templates` (بيانات وصفية + أمثلة curl جاهزة)
...
- **إنفاذ الميزانيات** — حدود ميزانية لكل مفتاح ولكل فريق تُنفَّذ عند البوابة مع قاطع
  دائرة تلقائي (وحدة تحكم إدارة الميزانيات: Enterprise)
- **حواجز وقائية** — محرّكات مدمجة (تصفية كلمات مفتاحية، طول المحتوى، كشف تسريب بيانات
  الاعتماد مع قاعدة افتراضية تُزرع تلقائياً) بالإضافة إلى سجلّ `RegisterEngine` قابل
  للإضافة؛ إجراءات block / log / mask مع إدارة CRUD وإعداد لكل نموذج
- **رموز PAT محددة النطاق** — رموز وصول شخصية لـ CI / الوصول الآلي مع واجهات قراءة فقط
  للاستخدام والميزانية والصحة
- **حماية SSRF** — حماية على مستوى الاتصال وإعادة التوجيه للطلبات الصادرة للمزوّدين
- **مرونة تشفير** — معيارية (SHA-256/RSA/AES) أو تشفير صيني وطني (SM3/SM2/SM4)

### القابلية للملاحظة

- **تحليلات الاستخدام** — استخدام التوكنات، وتتبّع التكلفة (يشمل تسعير الصور المتدرّج
  ومعاملات السعر لكل مفتاح)، ومقاييس الكمون، ومعدّلات أصابة التخزين المؤقت، وعدّادات
  التراجع/إعادة المحاولة لكل طلب
- **مقاييس Prometheus** — نقطة نهاية مقاييس مدمجة للمراقبة
- **OpenTelemetry** — دعم تتبّع موزّع
- **تسجيل منظّم** — تسجيل JSON بسياق الطلب
- **مواصفة OpenAPI مدمجة** — `/openapi.json` جاهزة للاستيراد في Postman وتوليد SDK
...
- **Playground للإدارة** — اختبر المحادثة والتدفق وتوليد الصور و TTS / التفريغ النصي
  والفيديو و Responses API من لوحة التحكم، عبر سلسلة التراجع الحقيقية
- **وضع تجريبي بلا تكلفة** — مزوّد وهمي اختياري + مفتاح تجريبي مدمج يشغّل صفحة تجربة
  عامة `/demo` دون أي إنفاق على المزوّدين
...
> **لا تملك مفتاح مزوّد بعد؟** فعّل وضع التجربة بلا تكلفة (`demo.enabled: true` في
> `config.yaml`) — مزوّد وهمي مع مفتاح تجريبي مدمج يشغّل صفحة تجربة عامة `/demo`
> لتجربة البوابة دون أي حساب لدى المزوّدين.
...
| `POST` | `/v1/videos` | توليد فيديو (استعلام عبر `GET /v1/videos/:id`، تنزيل عبر `/:id/content`) |
| `GET` | `/v1/usage` | استعلام ذاتي للحصة والاستخدام لحاملي المفاتيح |
| `GET` | `/v1/templates` | كتالوج قوالب الأوامر (بيانات وصفية + أمثلة curl) |
| `GET` | `/openapi.json` | مواصفة OpenAPI مدمجة — استوردها في Postman أو ولّد SDK |
...
| `GET` | `/admin/api/usage/stats` | إحصاءات الاستخدام (تشمل مقاييس الصور) + تصدير التسوية |
| `GET` | `/admin/api/routing/stats` | توزيع التوجيه: المُهيّأ مقابل الفعلي لكل مزوّد |
| `CRUD` | `/admin/api/error-rules` | قواعد تصنيف أخطاء الـ Failover |
| `CRUD` | `/admin/api/guardrails` | قواعد الحواجز (اختبار عبر `POST /:id/test`، بالإضافة إلى `/stats` و `/config`) |
| `CRUD` | `/admin/api/templates` | قوالب الأوامر (معاينة عبر `POST /:id/preview`) |
| `CRUD` | `/admin/api/pats` | رموز الوصول الشخصية |
| `POST` | `/admin/api/playground/*` | Playground: chat و stream و image و tts و transcribe و translate و video و responses |
| `GET/PUT` | `/admin/api/system/content-log` | تبديل فوري لتسجيل أجسام الطلب/الاستجابة |
| `CRUD` | `/admin/api/mcp/servers` | إدارة خوادم MCP |
| `GET` | `/admin/api/mcp/servers/:id/tools` | سرد الأدوات على خادم MCP |

الجداول تعرض أبرز النقاط لا كل شيء. تخدم البوابة مواصفة OpenAPI قابلة للاستيراد على
`/openapi.json`، ومرجع الـ API الكامل — بما يشمل أشكال الطلب/الاستجابة وأكواد الأخطاء
وترويسات `x-crosslink-fallback-*` — في [docs/api-reference.md](docs/api-reference.md).
...
- [x] ترجمة OpenAI Responses API (تدفق + غير تدفق) — المحرّك ومدخل Playground في
  Community؛ مسار `/v1/responses` العام يصدر مع الإصدارات التجارية
- [x] CAPTCHA منزلق مُستضاف ذاتياً (جاهز للعزل)
- [x] محرّكات حواجز مجتمعية (كلمات مفتاحية / طول / كشف بيانات اعتماد) مع إدارة CRUD
- [x] Playground مدمج للإدارة (محادثة، صور، صوت، فيديو، Responses)
- [x] وضع تجريبي بلا تكلفة (مزوّد وهمي + صفحة `/demo` عامة)
- [x] مواصفة OpenAPI مدمجة على `/openapi.json`
